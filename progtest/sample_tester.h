...
class CProducerSync : public CProducer{
  public:
    CProducerSync(std::function<void(AProducer, APriceList)> receiver): m_Receiver(receiver){}
...
class CProducerAsync : public CProducer{
  public:
    CProducerAsync(std::function<void(AProducer, APriceList)> receiver): m_Receiver(receiver){}
    virtual void sendPriceList(unsigned materialID) override;
    void start();
    void stop();
  private:
...
class CCustomerTest : public CCustomer{
  public:
    CCustomerTest(unsigned count): m_Count(count){}
